...
async def commit_bd(stock_data):
    ...
    try:
        async with conn.transaction():
            await conn.execute('''
                CREATE TABLE IF NOT EXISTS brazilian_stocks (
            ...
            await conn.execute('''
                INSERT INTO brazilian_stocks(ticket, price, timestamp)
                VALUES($1, $2, $3)
...
async def main():
    async with websockets.serve(register, "localhost", 6790):
